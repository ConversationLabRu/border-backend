...
export default class ReportService {
    ...
    static async getStatistic(borderCrossingId) {
        let attempt = 0;

        while (attempt < 5) {
            try {
                const response = await api.get(`/api/directions/borderCrossing/reports/statistic?borderCrossingId=${borderCrossingId}`);
                return response.data; // Успешный ответ, возвращаем данные
            } catch (error) {
                attempt += 1;
                console.error(`Attempt ${attempt} failed: ${error.message}`);
    ...
    static async getStatisticGraph(borderCrossingId) {
        let attempt = 0;

        while (attempt < 5) {
            try {
                const response = await api.get(`/api/directions/borderCrossing/reports/statisticGraph?borderCrossingId=${borderCrossingId}`);
                return response.data; // Успешный ответ, возвращаем данные
            } catch (error) {
                attempt += 1;
                console.error(`Attempt ${attempt} failed: ${error.message}`);
